...
To update the database use this command from the Web project folder (replace `ProductService` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ProductService.Infrastructure/ProductService.Infrastructure.csproj -s ProductService.Web.csproj
```
