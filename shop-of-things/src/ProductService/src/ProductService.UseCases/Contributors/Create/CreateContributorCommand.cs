...
using ProductService.Core.ContributorAggregate;
using FastEndpoints;
using MediatR;
using Microsoft.Extensions.Logging;

namespace ProductService.UseCases.Contributors.Create;
...
public class CreateContributorCommandHandler2 : CommandHandler<CreateContributorCommand2, Result<int>>
{
    ...
    public override async Task<Result<int>> ExecuteAsync(CreateContributorCommand2 request,
    ...
    : ICommandMiddleware<TCommand, TResult> where TCommand : FastEndpoints.ICommand<TResult>
{
        ...
        CancellationToken ct)
    {
        ...
        _logger.LogInformation("Handled {CommandName} with {Result} in {ms} ms", commandName, result,
