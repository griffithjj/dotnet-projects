...
To update the database use this command from the Web project folder (replace `OrderService` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../OrderService.Infrastructure/OrderService.Infrastructure.csproj -s OrderService.Web.csproj
```
